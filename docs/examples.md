...
Common logres patterns. For a guided first-run, see [the tutorial](tutorial.md). For every function signature, see [the reference](reference.md).
...
```sql
select logres.send('orders', 'order.created',
  '{"order_id": 42}'::jsonb);

select logres.send('orders', 'order.shipped',
...
select logres.send('orders', 'order.created', '{"order_id": 1}'::jsonb);
select logres.force_tick('orders');
select logres.ticker();

select * from logres.receive('orders', 'audit_logger', 100);
...
begin;
  ...
  select logres.ack((select distinct batch_id from msgs limit 1));
commit;
```
...
`logres.dlq_inspect()` lists entries for a queue. Replay a single row by its `dl_id`, or purge rows older than a given interval.
...
select logres.dlq_replay(42);

-- or drop entries older than 7 days
select logres.dlq_purge('orders', interval '7 days');
```
...
select * from logres.get_queue_info('orders');
-[ RECORD 1 ]------------+------------------------
queue_name               | orders
queue_ntables            | 3
queue_cur_table          | 2
...
select * from logres.get_consumer_info('orders', 'processor');
-[ RECORD 1 ]--+--------------
queue_name     | orders
...
`logres.status()` rolls up cron-job state, version, queue count, and consumer count into a single diagnostic view — run it first when something looks off.
