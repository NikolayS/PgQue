...
- **Producer** — anything that calls `insert_event` / `pg_current.send`. Any
...
- **Cross DB:** target-side batch/event tracking — record the `batch_id` or per-event ids on the target side and skip duplicates. pg_current does not ship a helper for this today.
...
Payload format is a producer/consumer contract — pg_current does not interpret it.
...
- `max_retries` — retry ceiling before a message goes to `pg_current.dead_letter`.
