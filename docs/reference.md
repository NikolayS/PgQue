...
**`queue_extra_maint` ownership check.** Before executing any function listed in a queue's `queue_extra_maint` column, `maint()` verifies that the function is owned by the same role that owns `maint()` itself (the install owner). Functions owned by any other role are skipped with a `WARNING` and not executed. This prevents a `pgque_admin` member from escalating privileges by registering an attacker-owned function in `queue_extra_maint` and triggering it via the `SECURITY DEFINER` `maint()`.

**Managed-DB compatibility.** The ownership check uses `pg_catalog.pg_get_userbyid()` rather than `pg_authid`, so it works on managed PostgreSQL services (RDS, Aurora, Cloud SQL, AlloyDB, Supabase, Neon, Crunchy Bridge) where non-superusers cannot `SELECT` from `pg_authid`.
...
`pgque.uninstall()` is revoked from `pgque_admin`, but not from PUBLIC — any role that can connect can still call it and drop the schema. Tighten with `revoke execute on function pgque.uninstall() from public;` if stricter control is required. `pgque.maint()` is explicitly revoked from PUBLIC and granted only to `pgque_admin`; it is a `SECURITY DEFINER` function with an ownership check on `queue_extra_maint` entries (see the `maint()` entry above). All other functions not in the table above are on PUBLIC `execute` by default (notably `start`, `stop`, `status`, `ticker`, `force_tick`, and the queue-management helpers `create_queue`, `drop_queue`, `set_queue_config`) — revoke and re-grant explicitly if your policy demands it.
