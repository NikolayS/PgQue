...
A hands-on walkthrough. With psql access to a Postgres 14+ instance and ten minutes, you can follow every step end to end. You will type SQL, see what comes back, and build an intuition for how pg_current moves messages.
...
Each step shows the exact SQL and the expected output. Your `msg_id` / `ev_id` / `pending_events` / `ev_new` numbers will differ from the examples — every `pg_current.force_tick` call skips the event sequence forward by about 1000, so exact numeric output depends on when you call it. Treat those numbers as illustrative. When transaction boundaries matter (and they matter — pg_current is snapshot-based), the text calls that out.

You can run every snippet in `psql` with `--no-psqlrc` and `PAGER=cat` if you want reproducible output. From the cloned repo, so `\i sql/pg_current.sql` resolves:

```bash
cd /path/to/pg_current
...
pg_current is a single SQL file. Install it inside a transaction so a failure leaves no half-built schema behind:
...
The install creates the `pg_current` schema, three roles (`pg_current_reader`, `pg_current_writer`, `pg_current_admin`), and every function you will call in the rest of this tutorial. See the [reference](reference.md) for the full surface.
...
`pg_current.send` also accepts a raw `text` payload — useful for protobuf, msgpack, or XML that you encode yourself. Untyped string literals like `'{"x":1}'` without the `::jsonb` cast resolve to the `text` overload. This tutorial stays on `jsonb` for clarity; see the [reference](reference.md) for the full overload rules and the NUL-byte caveat for binary payloads.
...
pg_current is **tick-based**, not row-claiming. Producers append events to the queue, but consumers do not see rows directly — they see **batches**. A batch is the set of events between two ticks. Until a tick happens, there is no batch boundary, so `pg_current.receive` has nothing to return.

In normal operation, a scheduler (`pg_cron` or an external loop) calls `pg_current.ticker()` every second and ticks happen continuously. In this tutorial you have not started a scheduler, so no tick has run yet.
...
For demos and tests, pg_current provides `pg_current.force_tick` to bypass the tick thresholds for one queue. It does **not** create the tick by itself — you still have to call `pg_current.ticker()` afterwards to produce the tick:
...
In production, `pg_cron` (or a small worker loop) calls `pg_current.ticker()` every second. `force_tick` exists for the situation here: advancing the queue without waiting on the ticker's lag threshold.
...
```sql
select batch_id from pg_current.receive('orders', 'processor', 100) limit 1 \gset
...
`pg_current.receive` = `next_batch` + `get_batch_events`. `pg_current.ack` = `finish_batch`. `pg_current.nack` = `event_retry` (with DLQ routing when `retry_count >= max_retries`). Both surfaces ship; the primitives are available for advanced use. See the [reference](reference.md) or the [concepts glossary](pgq-concepts.md).

Every row `pg_current.receive` returns is a `pg_current.message` composite: `msg_id` (PgQ's `ev_id`), `batch_id`, `type`, `payload` (text — cast to `jsonb` for JSON access), `retry_count` (NULL on first delivery), `created_at`, and four free-form `extra1..4` text columns.
...
```sql
select pg_current.set_queue_config('orders', 'max_retries', '2');
```
...
```sql
select pg_current.send('orders', '{"order_id": 43, "total": 10.00}'::jsonb);
select pg_current.force_tick('orders');
select pg_current.ticker();
select * from pg_current.receive('orders', 'processor', 100);
```
...
Now pretend the handler failed. `nack` takes the full `pg_current.message` row, so the natural pattern is a `do` block that receives and nacks in one place.
...
begin
    select * into v_msg from pg_current.receive('orders', 'processor', 1) limit 1;
    perform pg_current.nack(v_msg.batch_id, v_msg, '0 seconds'::interval, 'simulated failure');
    perform pg_current.ack(v_msg.batch_id);
end $$;
```

The event is now in PgQ's retry queue. Moving it back into the main event stream is a separate maintenance step: `pg_current.maint_retry_events()`. After that, the next tick makes it visible again:

```sql
select pg_current.maint_retry_events();
select pg_current.force_tick('orders');
select pg_current.ticker();
select * from pg_current.receive('orders', 'processor', 100);
```

In production, `pg_current.start()` schedules `maint_retry_events` on its own cadence — you never call it by hand. See [`pg_current.maint()`](reference.md#pg_currentmaint--integer) and the surrounding Lifecycle entries in the reference.
...
Keep nacking. You set `max_retries = 2` in step 7, and the message was just redelivered with `retry_count = 1`. On the next `nack` it becomes `retry_count = 2`; one more `nack` after that, and `nack` sees `retry_count >= max_retries` and routes the message to `pg_current.dead_letter` instead of the retry queue.
...
begin
    select * into v_msg from pg_current.receive('orders', 'processor', 1) limit 1;
    perform pg_current.nack(v_msg.batch_id, v_msg, '0 seconds'::interval, 'still failing');
    perform pg_current.ack(v_msg.batch_id);
end $$;
...
The second iteration sees `retry_count = 2` and routes to the DLQ instead of the retry queue. After it runs, `receive` returns nothing — the event has moved to `pg_current.dead_letter`.
...
```sql
select pg_current.dlq_replay(1);
```
...
```sql
select pg_current.dlq_purge('orders', '0 seconds'::interval);
```
...
from pg_current.get_queue_info('orders');
```
...
from pg_current.get_consumer_info('orders', 'processor');
```
...
------------+-------------+----------------------------------
 ...
 pg_current      | info        | [[your version]]
 pg_cron    | unavailable | pg_cron not installed -- call ...
 queues     | info        | 1 queues configured
 consumers  | info        | 1 active subscriptions
```

`status()` is the one-stop health check. If `pg_cron` is installed and `pg_current.start()` has been run, you will see `ticker` and `maintenance` rows with `scheduled` status and the cron job id.
...
With `pg_cron` available in the same database as pg_current:

```sql
select pg_current.start();
```

That one call schedules four cron jobs: `pg_current_ticker` every second, `pg_current_retry_events` every thirty seconds (moves `nack`'d events back into the main stream), `pg_current_maint` every thirty seconds (rotation step 1 and vacuum), and `pg_current_rotate_step2` every ten seconds (rotation step 2). Check them with `select * from pg_current.status();` or `select * from cron.job;`.
...
**pg_cron log hygiene.** `pg_cron` logs every job execution to `cron.job_run_details`. At the one-second ticker cadence, that table grows by roughly 3,600 rows per hour from pg_current alone, with no built-in purge.
...
select cron.schedule(
  'pg_current_purge_cron_log',
  '0 * * * *',
  $$delete from cron.job_run_details where end_time < now() - interval '1 day'$$
);
```

*Todo: a future `pg_current.start()` will warn about this overhead and offer to schedule the purge job.*

Without `pg_cron` at all, call `pg_current.ticker()` and `pg_current.maint()` from your application or an external scheduler (system `cron`, systemd, a worker loop) on the same cadence. The install is still useful — you provide the heartbeat yourself.
