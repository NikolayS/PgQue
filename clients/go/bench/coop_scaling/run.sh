...
#   EVENTS     events per run (default 5000)
...
for n in $N_VALUES; do
  echo "running -subconsumers=${n} -events=${EVENTS} -payload=${PAYLOAD} -runs=${RUNS}" >&2
...
FOOTER="PostgreSQL ${PG_VERSION} on ${MACHINE} -- ${EVENTS} events, ${PAYLOAD} B payload"
if [ -n "$PG_NOTE" ]; then
