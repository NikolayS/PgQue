...
// Client is the main pg_current client backed by a pgx connection pool.
type Client struct {
	pool *pgxpool.Pool
}
...
func Connect(ctx context.Context, dsn string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pg_current: connect: %w", err)
	}
...
func (c *Client) Send(ctx context.Context, queue string, ev Event) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("pg_current: marshal payload: %w", err)
	}
	typ := ev.Type
	if typ == "" {
		typ = "default"
	...
	err = c.pool.QueryRow(ctx,
		"SELECT pg_current.send($1, $2, $3::jsonb)", queue, typ, string(payload),
	).Scan(&eid)
	if err != nil {
		return 0, fmt.Errorf("pg_current: send: %w", err)
	}
	return eid, nil
}
...
func (c *Client) Receive(ctx context.Context, queue, consumer string, maxMessages int) ([]Message, error) {
	rows, err := c.pool.Query(ctx,
		"SELECT * FROM pg_current.receive($1, $2, $3)", queue, consumer, maxMessages)
	if err != nil {
		return nil, fmt.Errorf("pg_current: receive: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("pg_current: scan message: %w", err)
		}
		m.CreatedAt = createdAt
		msgs = append(msgs, m)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("pg_current: receive rows: %w", err)
	}
	return msgs, nil
}
...
func (c *Client) Ack(ctx context.Context, batchID int64) error {
	_, err := c.pool.Exec(ctx, "SELECT pg_current.ack($1)", batchID)
	if err != nil {
		return fmt.Errorf("pg_current: ack: %w", err)
	}
	return nil
}
