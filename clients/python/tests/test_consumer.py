import pytest
from pg_current import PgqueClient, Consumer, Message
...
def test_nack_retries_event(conn, setup_queue):
    ...
    conn.execute("SELECT pg_current.maint_retry_events('pytest_queue')")
    conn.execute("SELECT pg_current.force_tick('pytest_queue')")
    conn.execute("SELECT pg_current.ticker()")
    conn.commit()
