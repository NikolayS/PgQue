...
class LogresClient:
    ...
    def send(
        ...
        *,
        ...
        Maps to ``logres.send(queue, payload)`` or
        ``logres.send(queue, type, payload)``.
        ...
        Returns:
            The event ID assigned by logres.
        """
        ...
        if type != "default":
            row = self.conn.execute(
                "select logres.send(%s, %s, %s::jsonb)",
                (queue, type, payload),
            ).fetchone()
        else:
            row = self.conn.execute(
                "select logres.send(%s, %s::jsonb)",
                (queue, payload),
            ).fetchone()
    ...
    def send_batch(
        ...
        Maps to ``logres.send_batch(queue, type, payloads[])``.
        ...
        row = self.conn.execute(
            "select logres.send_batch(%s, %s, %s::jsonb[])",
    ...
    def receive(
        ...
        Maps to ``logres.receive(queue, consumer, max_messages)``.
        ...
        rows = self.conn.execute(
            "select * from logres.receive(%s, %s, %s)",
    ...
    def ack(self, batch_id: int) -> int:
        ...
        Maps to ``logres.ack(batch_id)`` which calls
        ``logres.finish_batch()``.  This advances the consumer past
        the entire batch.
        ...
        Returns:
            Result from ``logres.ack()``.
        """
        row = self.conn.execute(
            "select logres.ack(%s)", (batch_id,)
        ).fetchone()
        return row[0]
    ...
    def nack(
        ...
        Maps to ``logres.nack(batch_id, msg, retry_after, reason)``.
        ...
        self.conn.execute(
            "select logres.nack("
            "  %s,"
            "  ROW(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)::logres.message,"
            "  %s::interval,"
            "  %s"
            ")",
            (
