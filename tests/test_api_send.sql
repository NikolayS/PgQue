...
-- Test logres.send() and related functions
...
declare
  v_eid bigint;
begin
  perform logres.create_queue('test_send');
  perform logres.subscribe('test_send', 'c1');

  v_eid := logres.send('test_send', '{"key": "value"}'::jsonb);
...
declare
  v_eid bigint;
begin
  v_eid := logres.send('test_send', 'order.created', '{"id": 1}'::jsonb);
...
declare
  v_ids bigint[];
begin
  v_ids := logres.send_batch('test_send', 'batch.test', array[
    '{"n":1}'::jsonb,
    '{"n":2}'::jsonb,
    '{"n":3}'::jsonb
  ]);
...
declare
  v_eid bigint;
begin
  v_eid := logres.send('test_send', '{"raw":"text"}'::text);
...
declare
  v_eid bigint;
begin
  v_eid := logres.send('test_send', 'raw.binary', E'\\x01\\x02\\x03 not-json');
...
declare
  v_ids bigint[];
begin
  v_ids := logres.send_batch('test_send', 'batch.text', array[
    'opaque-1',
    'opaque-2',
    'opaque-3'
  ]::text[]);
...
declare
  v_ids bigint[];
begin
  v_ids := logres.send_batch('test_send', 'batch.empty', array[]::jsonb[]);
  ...
  v_ids := logres.send_batch('test_send', 'batch.empty', array[]::text[]);
...
declare
  v_count int;
begin
  perform logres.subscribe('test_send', 'c2');

  select count(*) into v_count from logres.get_consumer_info('test_send');
  assert v_count = 2, 'should have 2 consumers (c1 + c2), got ' || v_count;

  perform logres.unsubscribe('test_send', 'c2');
...
do $$
begin
  perform logres.unsubscribe('test_send', 'c1');
  perform logres.drop_queue('test_send');
