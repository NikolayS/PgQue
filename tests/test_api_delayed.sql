...
declare
  ...
  v_de_count bigint;
begin
  perform pg_current.create_queue('test_delayed');
  perform pg_current.register_consumer('test_delayed', 'c1');

  -- Send with future delivery (1 hour from now)
  v_id := pg_current.send_at('test_delayed', 'delayed.test',
  ...
  select count(*) into v_de_count from pg_current.delayed_events
  ...
  begin
    v_batch_id := pg_current.next_batch('test_delayed', 'c1');
  ...
  delete from pg_current.delayed_events where de_queue_name = 'test_delayed';
  perform pg_current.unregister_consumer('test_delayed', 'c1');
  perform pg_current.drop_queue('test_delayed');
end $$;
...
declare
  ...
  v_ev_count int := 0;
begin
  perform pg_current.create_queue('test_delayed2');
  perform pg_current.register_consumer('test_delayed2', 'c1');

  v_id := pg_current.send_at('test_delayed2', 'immediate.test',
  ...
  perform pg_current.unregister_consumer('test_delayed2', 'c1');
  perform pg_current.drop_queue('test_delayed2');
end $$;
...
declare
  ...
  v_ev record;
begin
  perform pg_current.create_queue('test_delayed3');
  perform pg_current.register_consumer('test_delayed3', 'c1');

  -- Insert a delayed event that is already due (past time)
  insert into pg_current.delayed_events (de_queue_name, de_deliver_at, de_type, de_data)
  values ('test_delayed3', now() - interval '1 second', 'delivered.test', '{"delivered":true}');

  -- Run maint_deliver_delayed
  select pg_current.maint_deliver_delayed() into v_count;
  assert v_count >= 1, 'should deliver at least 1 event, got ' || v_count;

  -- Now ticker + receive should get the event
  perform pg_current.ticker();
  v_batch_id := pg_current.next_batch('test_delayed3', 'c1');
  ...
  perform pg_current.unregister_consumer('test_delayed3', 'c1');
  perform pg_current.drop_queue('test_delayed3');
end $$;

-- Test 4: pg_current.maint() delivers delayed events end-to-end
do $$
declare
  v_delivered int;
begin
  perform pg_current.create_queue('test_maint_delayed');
  perform pg_current.register_consumer('test_maint_delayed', 'c1');
  ...
  assert not exists (
    select 1 from pg_current.delayed_events where de_queue_name = 'test_maint_delayed'
  ), 'delayed event should be gone after maint()';

  perform pg_current.unregister_consumer('test_maint_delayed', 'c1');
  perform pg_current.drop_queue('test_maint_delayed');
  raise notice 'PASS: pg_current.maint() delivers delayed events';
end $$;
