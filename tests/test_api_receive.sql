...
do $$
begin
  perform logres.create_queue('test_recv');
  perform logres.register_consumer('test_recv', 'c1');
end $$;

-- Step 2: insert event (separate transaction)
do $$
begin
  perform logres.insert_event('test_recv', 'test.type', '{"key":"val"}');
end $$;
...
declare
  ...
  v_count int := 0;
begin
  for v_msg in select * from logres.receive('test_recv', 'c1', 10)
  loop
    v_count := v_count + 1;
...
do $$
begin
  perform logres.create_queue('test_recv_partial');
  perform logres.register_consumer('test_recv_partial', 'c1');
end $$;

do $$
begin
  perform logres.insert_event('test_recv_partial', 'test.type', '{"n":1}');
  perform logres.insert_event('test_recv_partial', 'test.type', '{"n":2}');
  perform logres.insert_event('test_recv_partial', 'test.type', '{"n":3}');
end $$;

do $$
begin
  perform logres.force_tick('test_recv_partial');
  perform logres.ticker();
end $$;
...
declare
  ...
  v_batch_id bigint;
begin
  for v_msg in select * from logres.receive('test_recv_partial', 'c1', 1)
  loop
    v_count := v_count + 1;
...
declare
  ...
  v_count int := 0;
begin
  for v_msg in select * from logres.receive('test_recv_partial', 'c1', 10)
  loop
    v_count := v_count + 1;
  end loop;
...
do $$
begin
  perform logres.create_queue('test_recv_text');
  perform logres.register_consumer('test_recv_text', 'c1');
end $$;
...
begin
  ...
  perform logres.send('test_recv_text', 'json.raw',   '{"b":2,"a":1}'::text);
  perform logres.send('test_recv_text', 'bin.raw',    E'\\x01\\x02opaque'::text);
  perform logres.send('test_recv_text', 'json.nocast', '{"z":9,"m":5,"a":1}');
end $$;

do $$
begin
  perform logres.force_tick('test_recv_text');
  perform logres.ticker();
end $$;
...
begin
  for v_msg in select * from logres.receive('test_recv_text', 'c1', 10)
  loop
    v_batch_id := v_msg.batch_id;
...
do $$
begin
  perform logres.unregister_consumer('test_recv', 'c1');
  perform logres.drop_queue('test_recv');
  perform logres.unregister_consumer('test_recv_partial', 'c1');
  perform logres.drop_queue('test_recv_partial');
  perform logres.unregister_consumer('test_recv_text', 'c1');
  perform logres.drop_queue('test_recv_text');
