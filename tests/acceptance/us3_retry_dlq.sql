...
-- ==============================
do $$ begin
  perform pg_current.create_queue('us3_jobs', '{"max_retries": 2}'::jsonb);
  perform pg_current.subscribe('us3_jobs', 'worker');
end $$;

-- Send event
do $$ begin
  perform pg_current.send('us3_jobs', 'job.process', '{"task":"test"}'::jsonb);
end $$;

-- Ticker (force_tick bypasses throttle)
do $$ begin
  perform pg_current.force_tick('us3_jobs');
...
declare
  ...
  v_count int := 0;
begin
  for v_msg in select * from pg_current.receive('us3_jobs', 'worker', 10)
  loop
    v_count := v_count + 1;
    ...
    perform pg_current.nack(v_msg.batch_id, v_msg, '0 seconds'::interval, 'transient failure');
  end loop;
...
begin
  ...
  loop
    ...
    perform pg_current.nack(v_msg.batch_id, v_msg, '0 seconds'::interval, 'permanent failure');
  end loop;
...
begin
  ...
  select * into v_dl from pg_current.dlq_inspect('us3_jobs', 100) limit 1;
...
do $$ begin
  ...
  where dl_queue_id = (select queue_id from pg_current.queue where queue_name = 'us3_jobs');
  perform pg_current.unsubscribe('us3_jobs', 'worker');
  perform pg_current.drop_queue('us3_jobs');
end $$;
