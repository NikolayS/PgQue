...
-- confirming pg_current handles real workloads without dead-tuple bloat.
-- SPECx.md section 13.3
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.

-- Setup: create queue and subscribe consumer
do $$ begin
  perform pg_current.create_queue('us5_ingest');
  perform pg_current.subscribe('us5_ingest', 'etl');
end $$;
...
begin
  for i in 1..10000 loop
    v_id := pg_current.insert_event('us5_ingest', 'data.load',
      '{"seq":' || i || '}');
  end loop;
...
do $$ begin
  perform pg_current.force_tick('us5_ingest');
...
declare
  ...
  v_batch_id bigint;
begin
  for v_msg in select * from pg_current.receive('us5_ingest', 'etl', 20000)
  loop
    v_count := v_count + 1;
...
-- Verify: no dead tuples in pg_current event tables
...
begin
  ...
  -- Check dead tuples across all pg_current event tables
...
-- Teardown
do $$ begin
  perform pg_current.unsubscribe('us5_ingest', 'etl');
  perform pg_current.drop_queue('us5_ingest');
end $$;
