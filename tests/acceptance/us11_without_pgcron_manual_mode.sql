...
-- On a fresh database without pg_cron, logres should install and basic
...
begin
  begin
    ...
    assert false, 'logres.start() should fail without pg_cron';
  exception when raise_exception then
    assert sqlerrm like '%logres itself works without pg_cron%',
...
do $$
begin
  perform logres.create_queue('us11_manual');
  perform logres.subscribe('us11_manual', 'worker');
end $$;

do $$
begin
  perform logres.send('us11_manual', 'demo.event', '{"ok":true}'::jsonb);
end $$;

do $$
begin
  perform logres.force_tick('us11_manual');
...
declare
  ...
  v_batch_id bigint;
begin
  for v_msg in select * from logres.receive('us11_manual', 'worker', 10)
  loop
    v_count := v_count + 1;
...
begin
  ...
  where component = 'pg_cron'
    ...
    and detail like '%logres.ticker()%logres.maint()%';
...
do $$
begin
  perform logres.unsubscribe('us11_manual', 'worker');
  perform logres.drop_queue('us11_manual');
end $$;
