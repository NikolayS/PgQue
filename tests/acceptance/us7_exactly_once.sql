...
do $$ begin
  perform pg_current.create_queue('us7_payments');
  perform pg_current.subscribe('us7_payments', 'processor');
end $$;
...
-- Send event
do $$ begin
  perform pg_current.send('us7_payments', 'payment.process',
    '{"amount":100,"id":"pay_001"}'::jsonb);
end $$;

-- Tick
do $$ begin
  perform pg_current.force_tick('us7_payments');
...
declare
  ...
  v_batch_id bigint;
begin
  for v_msg in select * from pg_current.receive('us7_payments', 'processor', 10)
  loop
    v_count := v_count + 1;
...
do $$ begin
  perform pg_current.unsubscribe('us7_payments', 'processor');
  perform pg_current.drop_queue('us7_payments');
end $$;
