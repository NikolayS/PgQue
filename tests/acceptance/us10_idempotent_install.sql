...
-- As an operator, I want to re-run pg_current.sql without losing
...
do $$ begin
  perform pg_current.create_queue('us10_orders');
  perform pg_current.create_queue('us10_logs');
  perform pg_current.subscribe('us10_orders', 'fulfillment');
  perform pg_current.subscribe('us10_orders', 'analytics');
  perform pg_current.subscribe('us10_logs', 'archiver');
end $$;

-- Insert events
do $$ begin
  perform pg_current.insert_event('us10_orders', 'order.created', '{"id":1}');
  perform pg_current.insert_event('us10_orders', 'order.created', '{"id":2}');
  perform pg_current.insert_event('us10_orders', 'order.created', '{"id":3}');
  perform pg_current.insert_event('us10_logs', 'log.entry', '{"msg":"hello"}');
end $$;

-- Tick to create proper state
do $$ begin
  perform pg_current.force_tick('us10_orders');
  perform pg_current.force_tick('us10_logs');
...
declare
  ...
  v_count int := 0;
begin
  for v_msg in select * from pg_current.receive('us10_orders', 'fulfillment', 100)
  loop
    v_count := v_count + 1;
...
declare
  ...
  v_last_tick bigint;
begin
  select count(*) into v_queue_count from pg_current.queue;
  select count(*) into v_sub_count from pg_current.subscription;
...
do $$ begin
  raise notice 'PASS: US-10 pg_current.sql re-run completed without errors';
end $$;
...
declare
  v_id bigint;
begin
  v_id := pg_current.send('us10_orders', 'order.created',
...
declare
  ...
  v_batch_id bigint;
begin
  for v_msg in select * from pg_current.receive('us10_orders', 'analytics', 100)
  loop
    v_count := v_count + 1;
    v_batch_id := v_msg.batch_id;
  end loop;
...
do $$ begin
  perform pg_current.unsubscribe('us10_orders', 'fulfillment');
  perform pg_current.unsubscribe('us10_orders', 'analytics');
  perform pg_current.unsubscribe('us10_logs', 'archiver');
  perform pg_current.drop_queue('us10_orders');
  perform pg_current.drop_queue('us10_logs');
end $$;
