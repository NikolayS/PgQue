...
do $$
begin
  perform logres.create_queue('test_events');
  perform logres.register_consumer('test_events', 'c1');
end $$;
...
declare
  v_eid bigint;
begin
  v_eid := logres.insert_event('test_events', 'test.type', '{"key":"value"}');
...
declare
  ...
  v_ev record;
begin
  v_batch_id := logres.next_batch('test_events', 'c1');
...
do $$
begin
  perform logres.unregister_consumer('test_events', 'c1');
  perform logres.drop_queue('test_events');
