...
do $$
begin
  perform pg_current.create_queue('test_events');
  perform pg_current.register_consumer('test_events', 'c1');
end $$;
...
declare
  v_eid bigint;
begin
  v_eid := pg_current.insert_event('test_events', 'test.type', '{"key":"value"}');
...
declare
  ...
  v_ev record;
begin
  v_batch_id := pg_current.next_batch('test_events', 'c1');
...
do $$
begin
  perform pg_current.unregister_consumer('test_events', 'c1');
  perform pg_current.drop_queue('test_events');
