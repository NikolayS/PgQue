...
do $$
begin
  perform pg_current.create_queue('test_rotation');
  perform pg_current.set_queue_config('test_rotation', 'rotation_period', '1 second');
  perform pg_current.ticker('test_rotation');
  perform pg_current.insert_event('test_rotation', 'test', 'data');
  perform pg_current.ticker('test_rotation');
...
declare
  v_result integer;
begin
  select pg_current.maint_rotate_tables_step1('test_rotation') into v_result;
...
declare
  v_cur integer;
begin
  perform pg_current.maint_rotate_tables_step2();

  select queue_cur_table into v_cur
  from pg_current.queue where queue_name = 'test_rotation';
...
-- Rotation 2: step1
do $$ begin perform pg_current.maint_rotate_tables_step1('test_rotation'); end; $$;
...
begin
  perform pg_current.maint_rotate_tables_step2();
  select queue_cur_table into v_cur from pg_current.queue where queue_name = 'test_rotation';
...
do $$
begin
  perform pg_current.drop_queue('test_rotation');
