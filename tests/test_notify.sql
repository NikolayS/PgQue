...
begin
  ...
  for v_src in
    ...
    where n.nspname = 'logres' and p.proname = 'ticker'
  loop
    if v_src like '%pg_notify%' and v_src like '%logres_%' then
      v_found := true;
    end if;
  end loop;

  assert v_found, 'at least one ticker overload should contain pg_notify on logres_ channel';
