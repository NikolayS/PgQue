-- test_observability.sql -- logres observability functions
...
do $$ begin
  perform logres.create_queue('obs_queue');
  perform logres.register_consumer('obs_queue', 'obs_consumer');

  -- Insert some events
  perform logres.insert_event('obs_queue', 'obs.test', '{"n":1}');
  perform logres.insert_event('obs_queue', 'obs.test', '{"n":2}');
  perform logres.insert_event('obs_queue', 'obs.test', '{"n":3}');
end $$;

do $$ begin
  perform logres.force_tick('obs_queue');
...
begin
  ...
  assert v_depth_found, 'otel_metrics() should include logres.queue.depth';
...
declare
  v_count int;
begin
  select count(*) into v_count from logres.stuck_consumers('0 seconds'::interval);
...
declare
  v_count int;
begin
  select count(*) into v_count from logres.in_flight('obs_queue');
...
begin
  ...
  from logres.throughput('obs_queue', '1 hour'::interval, '5 minutes'::interval);
...
begin
  select count(*) into v_count
  from logres.error_rate('obs_queue', '1 hour'::interval, '5 minutes'::interval);
...
-- Teardown
do $$ begin
  perform logres.unregister_consumer('obs_queue', 'obs_consumer');
  perform logres.drop_queue('obs_queue');
end $$;
