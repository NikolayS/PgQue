-- pg_current delayed delivery: send_at() and maint_deliver_delayed()
...
create table if not exists pg_current.delayed_events (
...
create index if not exists de_deliver_idx
    on pg_current.delayed_events (de_deliver_at);
...
create or replace function pg_current.send_at(
...
--
-- Called by pg_current.maint(). Deletes rows from delayed_events where
-- de_deliver_at <= now() and inserts them into the target queue
-- via insert_event().
create or replace function pg_current.maint_deliver_delayed()
returns integer as $$
declare
    ev record;
    cnt integer := 0;
...
-- requirement). pg_current.start() schedules step2 as its own pg_cron job.
--
...
begin
    ...
    loop
        if f.func_name = 'pg_current.maint_rotate_tables_step2' then
            -- Step2 runs in a separate pg_cron job (pg_current_rotate_step2)
            continue;
    ...
    select pg_current.maint_deliver_delayed() into r;
    total := total + r;
