-- logres delayed delivery: send_at() and maint_deliver_delayed()
...
create table if not exists logres.delayed_events (
...
create index if not exists de_deliver_idx
    on logres.delayed_events (de_deliver_at);
...
create or replace function logres.send_at(
...
--
-- Called by logres.maint(). Deletes rows from delayed_events where
-- de_deliver_at <= now() and inserts them into the target queue
-- via insert_event().
create or replace function logres.maint_deliver_delayed()
returns integer as $$
declare
    ev record;
    cnt integer := 0;
...
-- requirement). logres.start() schedules step2 as its own pg_cron job.
--
...
begin
    ...
    loop
        if f.func_name = 'logres.maint_rotate_tables_step2' then
            -- Step2 runs in a separate pg_cron job (logres_rotate_step2)
            continue;
    ...
    select logres.maint_deliver_delayed() into r;
    total := total + r;
