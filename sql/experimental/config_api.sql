...
-- pg_current.create_queue(queue, options) -- JSONB overload
create or replace function pg_current.create_queue(i_queue text, i_options jsonb)
returns integer as $$
declare
    v_ret integer;
    v_key text;
    v_val text;
begin
    v_ret := pg_current.create_queue(i_queue);
...
create or replace function pg_current.pause_queue(i_queue text)
returns void as $$
begin
...
create or replace function pg_current.resume_queue(i_queue text)
returns void as $$
begin
    perform pg_current.set_queue_config(i_queue, 'ticker_paused', 'false');
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;
