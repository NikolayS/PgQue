-- pg_current.sql -- PgQ Universal Edition
-- Version: 1.0.0-dev
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
-- Includes code derived from PgQ (ISC license, Marko Kreen / Skype Technologies OU).
--
-- Install: \i pg_current.sql
-- Start:   SELECT pg_current.start();
-- Usage:   See https://github.com/NikolayS/pg_current

create schema if not exists pg_current;

-- ======================================================================
-- Section 1: Tables (derived from PgQ)
-- Origin: pgq/structure/tables.sql
--
-- pg_current transformations applied:
--   1. Schema rename: pgq → pg_current (all identifiers, grants, references)
...
--
-- Overview:
--      pg_current.queue                   - Queue configuration
--      pg_current.consumer                - Consumer names
--      pg_current.subscription            - Consumer registrations
--      pg_current.tick                    - Per-queue snapshots (ticks)
--      pg_current.event_*                 - Data tables
--      pg_current.retry_queue             - Events to be retried later
--
-- 
-- Standard triggers store events in the pg_current.event_* data tables
-- There is one top event table pg_current.event_<queue_id> for each queue
-- inherited from pg_current.event_template wuith three tables for actual data
-- pg_current.event_<queue_id>_0 to pg_current.event_<queue_id>_2.
--
...
-- ----------------------------------------------------------------------
create table if not exists pg_current.consumer (
        co_id       serial,
...
--      queue_disable_insert        - disallow pg_current.insert_event()
...
-- ----------------------------------------------------------------------
create table if not exists pg_current.queue (
        queue_id                    serial,
        ...
        queue_switch_step1          bigint      not null default pg_current_xact_id()::text::bigint, -- pg_current transformation: txid_current()→pg_current_xact_id()::text::bigint (PG14+)
...
-- ----------------------------------------------------------------------
create table if not exists pg_current.tick (
        ...
        constraint tick_queue_fkey foreign key (tick_queue)
                                   references pg_current.queue (queue_id)
);

-- ----------------------------------------------------------------------
-- Sequence: pg_current.batch_id_seq
--
--      Sequence for batch id's.
-- ----------------------------------------------------------------------
create sequence if not exists pg_current.batch_id_seq;

-- ----------------------------------------------------------------------
-- Table: pg_current.subscription
--
...
-- ----------------------------------------------------------------------
create table if not exists pg_current.subscription (
        ...
        constraint sub_consumer_fkey foreign key (sub_consumer)
                                   references pg_current.consumer (co_id)
);

-- ----------------------------------------------------------------------
-- Table: pg_current.event_template
--
...
-- ----------------------------------------------------------------------
create table if not exists pg_current.event_template (
        ev_id               bigint          not null,
        ev_time             timestamptz     not null,

        ev_txid             xid8          not null default pg_current_xact_id(), -- pg_current transformation: bigint→xid8 (needed for pg_visible_in_snapshot)
        ev_owner            int4,
        ev_retry            int4,
...
--      *                       - same as pg_current.event_template
-- ----------------------------------------------------------------------
create table if not exists pg_current.retry_queue (
    ev_retry_after          timestamptz     not null,
    ev_queue                int4            not null,

    like pg_current.event_template,

    constraint rq_pkey primary key (ev_owner, ev_id),
    constraint rq_queue_id_fkey foreign key (ev_queue)
                             references pg_current.queue (queue_id)
);
alter table pg_current.retry_queue alter column ev_owner set not null;
alter table pg_current.retry_queue alter column ev_txid drop not null;
create index if not exists rq_retry_idx on pg_current.retry_queue (ev_retry_after);
...
--
-- pg_current transformations applied:
--   1. Schema rename: pgq → pg_current
...
--   5. SECURITY DEFINER functions get SET search_path = pg_current, pg_catalog
--   6. pgq_node/Londiste hooks removed from maint_operations
--   7. pg_notify() injected into ticker for LISTEN/NOTIFY wakeup
-- ======================================================================


create or replace function pg_current.upgrade_schema()
returns int4 as $$
...
begin
    ...
    -- pg_current.subscription.sub_last_tick: NOT NULL -> NULL
    perform 1 from information_schema.columns
      where table_schema = 'pg_current'
        and table_name = 'subscription'
        and column_name ='sub_last_tick'
        and is_nullable = 'NO';
    if found then
        alter table pg_current.subscription
    ...
    -- create roles
    perform 1 from pg_catalog.pg_roles where rolname = 'pg_current_reader';
    if not found then
        create role pg_current_reader;
        cnt := cnt + 1;
    end if;
    perform 1 from pg_catalog.pg_roles where rolname = 'pg_current_writer';
    if not found then
        create role pg_current_writer;
        cnt := cnt + 1;
    end if;
    perform 1 from pg_catalog.pg_roles where rolname = 'pg_current_admin';
    if not found then
        create role pg_current_admin in role pg_current_reader, pg_current_writer;
        cnt := cnt + 1;
    end if;

    perform 1 from pg_attribute
        where attrelid = 'pg_current.queue'::regclass
          and attname = 'queue_extra_maint';
    if not found then
        alter table pg_current.queue add column queue_extra_maint text[];
    end if;
...
create or replace function pg_current.batch_event_sql(x_batch_id bigint)
returns text as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.batch_event_sql(1)
...
begin
    select s.sub_last_tick, s.sub_next_tick, s.sub_id, s.sub_queue,
        ...
        into batch
        from pg_current.subscription s, pg_current.tick last, pg_current.tick cur
    ...
    -- must match pg_current.event_template
    ...
    sql := '';
    for rec in
        select xtbl from pg_current.batch_event_tables(x_batch_id) xtbl
    loop
        tbl := pg_current.quote_fqname(rec.xtbl);
        -- this gets newer queries that definitely are not in prev_snapshot
        part := select_fields
            || ' from pg_current.tick cur, pg_current.tick last, ' || tbl || ' ev '
...
create or replace function pg_current.batch_event_tables(x_batch_id bigint)
returns setof text as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.batch_event_tables(1)
--
...
begin
    select
        ...
        into batch
        from pg_current.tick last, pg_current.tick cur, pg_current.subscription s, pg_current.queue q
...
create or replace function pg_current.event_retry_raw(
    x_queue text,
    x_consumer text,
...
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.event_retry_raw(12)
--
...
begin
    select sub_id, queue_event_seq, sub_queue into q
      from pg_current.consumer, pg_current.queue, pg_current.subscription
    ...
    insert into pg_current.retry_queue (ev_retry_after, ev_queue,
...
$$ language plpgsql security definer set search_path = pg_current, pg_catalog; -- pg_current transformation: pin search_path (SECURITY DEFINER hardening)

create or replace function pg_current.find_tick_helper(
    in i_queue_id int4,
...
as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.find_tick_helper(6)
--
--      Helper function for pg_current.next_batch_custom() to do extended tick search.
-- ----------------------------------------------------------------------
declare
    sure    boolean;
    can_set boolean;
...
create or replace function pg_current.ticker(i_queue_name text, i_tick_id bigint, i_orig_timestamp timestamptz, i_event_seq bigint)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.ticker(3)
--
...
-- ----------------------------------------------------------------------
begin
    insert into pg_current.tick (tick_queue, tick_id, tick_time, tick_event_seq)
    select queue_id, i_tick_id, i_orig_timestamp, i_event_seq
        from pg_current.queue
    ...
    perform pg_current.seq_setval(queue_tick_seq, i_tick_id),
            pg_current.seq_setval(queue_event_seq, i_event_seq)
        from pg_current.queue
        where queue_name = i_queue_name;


    -- pg_current: notify listeners after tick
    perform pg_notify('pg_current_' || i_queue_name, i_tick_id::text); -- pg_current transformation: LISTEN/NOTIFY wakeup (not in original PgQ)
    return i_tick_id;
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;


create or replace function pg_current.ticker(i_queue_name text)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.ticker(1)
--
...
begin
    select queue_id, queue_tick_seq, queue_external_ticker,
            ...
            pg_current.seq_getval(queue_event_seq) as event_seq,
            queue_ticker_paused
        into q
        from pg_current.queue where queue_name = i_queue_name;
    if not found then
    ...
    insert into pg_current.tick (tick_queue, tick_id, tick_event_seq)
        values (q.queue_id, nextval(q.queue_tick_seq), q.event_seq);


    -- pg_current: notify listeners after tick
    perform pg_notify('pg_current_' || i_queue_name, currval(q.queue_tick_seq)::text); -- pg_current transformation: LISTEN/NOTIFY wakeup (not in original PgQ)
    return currval(q.queue_tick_seq);
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

create or replace function pg_current.ticker() returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.ticker(0)
--
...
begin
    res := 0;
    for q in
        select queue_name from pg_current.queue
            where not queue_external_ticker
                  and not queue_ticker_paused
            order by queue_name
    loop
        if pg_current.ticker(q.queue_name) > 0 then
            res := res + 1;
        end if;
    end loop;
    return res;
...
create or replace function pg_current.maint_retry_events()
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.maint_retry_events(0)
--
...
begin
    ...
    lock table pg_current.retry_queue in share update exclusive mode;

    for rec in
        select queue_name,
               ev_id, ev_time, ev_owner, ev_retry, ev_type, ev_data,
               ev_extra1, ev_extra2, ev_extra3, ev_extra4
          from pg_current.retry_queue, pg_current.queue
    ...
    loop
        cnt := cnt + 1;
        perform pg_current.insert_event_raw(rec.queue_name,
        ...
        delete from pg_current.retry_queue
...
create or replace function pg_current.maint_rotate_tables_step1(i_queue_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.maint_rotate_tables_step1(1)
--
...
begin
    ...
    select * from pg_current.queue into cf
        where queue_name = i_queue_name
    ...
    begin
        execute 'lock table ' || pg_current.quote_fqname(tbl) || ' nowait';
        execute 'truncate ' || pg_current.quote_fqname(tbl);
    exception
    ...
    -- 1. we want the pg_current.tick table to be big, to avoid Postgres
...
create or replace function pg_current.maint_rotate_tables_step2()
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.maint_rotate_tables_step2(0)
--
--      Stores the txid when the rotation was visible.  It should be
--      called in separate transaction than pg_current.maint_rotate_tables_step1()
-- ----------------------------------------------------------------------
begin
    update pg_current.queue
...
create or replace function pg_current.maint_tables_to_vacuum()
returns setof text as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.maint_tables_to_vacuum(0)
--
...
begin
    ...
    for scm, tbl in values
        ('pg_current', 'subscription'),
        ('pg_current', 'consumer'),
        ('pg_current', 'queue'),
        ('pg_current', 'tick'),
        ('pg_current', 'retry_queue'),
...
create or replace function pg_current.maint_operations(out func_name text, out func_arg text)
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.maint_operations(0)
--
...
begin
    -- rotate step 1
    nrot := 0;
    func_name := 'pg_current.maint_rotate_tables_step1';
    for func_arg in
        select queue_name from pg_current.queue
    ...
    -- rotate step 2
    if nrot = 0 then
        select count(1) from pg_current.queue
            where queue_rotation_period is not null
                and queue_switch_step2 is null
            into nrot;
    end if;
    if nrot > 0 then
        func_name := 'pg_current.maint_rotate_tables_step2';
        func_arg := NULL;
        return next;
    end if;
    ...
    if found then
        -- add extra ops
        for func_arg, ops in
            select q.queue_name, queue_extra_maint from pg_current.queue q
    ...
    func_name := 'vacuum';
    for func_arg in
        select * from pg_current.maint_tables_to_vacuum()
    loop
        return next;
    end loop;
...
create or replace function pg_current.grant_perms(x_queue_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.grant_perms(1)
--
...
declare
    ...
    part_table  text;
begin
    select * from pg_current.queue into q
    ...
    execute 'grant select on ' || pg_current.quote_fqname(q.queue_tick_seq) || ' to public';

    -- event seq
    execute 'grant select on ' || pg_current.quote_fqname(q.queue_event_seq) || ' to public';
    execute 'grant usage on ' || pg_current.quote_fqname(q.queue_event_seq) || ' to pg_current_admin';

    -- set grants on parent table
    perform pg_current._grant_perms_from('pg_current', 'event_template', dst_schema, dst_table);

    -- set grants on real event tables
    for i in 0 .. q.queue_ntables - 1 loop
        part_table := dst_table  || '_' || i::text;
        perform pg_current._grant_perms_from('pg_current', 'event_template', dst_schema, part_table);
    end loop;
...
create or replace function pg_current._grant_perms_from(src_schema text, src_table text, dst_schema text, dst_table text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.grant_perms_from(1)
--
...
create or replace function pg_current.tune_storage(i_queue_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.tune_storage(1)
--
...
create or replace function pg_current.force_tick(i_queue_name text)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.force_tick(2)
--
...
--
--      Such function is needed because paraller calls of pg_current.ticker() are
...
begin
    ...
    select tick_id into t
      from pg_current.tick, pg_current.queue
...
create or replace function pg_current.seq_getval(i_seq_name text)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.seq_getval(1)
--
...
create or replace function pg_current.seq_setval(i_seq_name text, i_new_value int8)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.seq_setval(2)
--
...
declare
    ...
    fqname  text;
begin
    fqname := pg_current.quote_fqname(i_seq_name);

    res := pg_current.seq_getval(i_seq_name);
...
create or replace function pg_current.quote_fqname(i_name text)
returns text as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.quote_fqname(1)
--
...
create or replace function pg_current.create_queue(i_queue_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.create_queue(1)
--
...
--      1 - queue created
-- Calls:
--      pg_current.grant_perms(i_queue_name);
--      pg_current.ticker(i_queue_name);
--      pg_current.tune_storage(i_queue_name);
-- Tables directly manipulated:
--      insert - pg_current.queue
--      create - pg_current.event_N () inherits (pg_current.event_template)
--      create - pg_current.event_N_0 .. pg_current.event_N_M () inherits (pg_current.event_N)
-- ----------------------------------------------------------------------
declare
    tblpfx   text;
    tblname  text;
...
begin
    ...
    -- check if exists
    perform 1 from pg_current.queue where queue_name = i_queue_name;
    if found then
        return 0;
    end if;

    -- insert event
    id := nextval('pg_current.queue_queue_id_seq');
    tblpfx := 'pg_current.event_' || id::text;
    idxpfx := 'event_' || id::text;
    tick_seq := 'pg_current.event_' || id::text || '_tick_seq';
    ev_seq := 'pg_current.event_' || id::text || '_id_seq';
    insert into pg_current.queue (queue_id, queue_name,
            queue_data_pfx, queue_event_seq, queue_tick_seq)
        values (id, i_queue_name, tblpfx, ev_seq, tick_seq);

    select queue_ntables into n_tables from pg_current.queue
        where queue_id = id;

    -- create seqs
    execute 'CREATE SEQUENCE ' || pg_current.quote_fqname(tick_seq);
    execute 'CREATE SEQUENCE ' || pg_current.quote_fqname(ev_seq);

    -- create data tables
    execute 'CREATE TABLE ' || pg_current.quote_fqname(tblpfx) || ' () '
            || ' INHERITS (pg_current.event_template)';
    for i in 0 .. (n_tables - 1) loop
        tblname := tblpfx || '_' || i::text;
        idxname := idxpfx || '_' || i::text || '_txid_idx';
        execute 'CREATE TABLE ' || pg_current.quote_fqname(tblname) || ' () '
                || ' INHERITS (' || pg_current.quote_fqname(tblpfx) || ')';
        execute 'ALTER TABLE ' || pg_current.quote_fqname(tblname) || ' ALTER COLUMN ev_id '
                || ' SET DEFAULT nextval(' || quote_literal(ev_seq) || ')';
        execute 'create index ' || quote_ident(idxname) || ' on '
                || pg_current.quote_fqname(tblname) || ' (ev_txid)';
    end loop;

    perform pg_current.grant_perms(i_queue_name);

    perform pg_current.ticker(i_queue_name);

    perform pg_current.tune_storage(i_queue_name);

    return 1;
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

create or replace function pg_current.drop_queue(x_queue_name text, x_force bool)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.drop_queue(2)
--
...
--      1 - success
-- Calls:
--      pg_current.unregister_consumer(queue_name, consumer_name)
--      perform pg_current.ticker(i_queue_name);
--      perform pg_current.tune_storage(i_queue_name);
-- Tables directly manipulated:
--      delete - pg_current.queue
--      drop - pg_current.event_N (), pg_current.event_N_0 .. pg_current.event_N_M 
-- ----------------------------------------------------------------------
declare
    tblname  text;
    q record;
...
begin
    -- check if exists
    select * into q from pg_current.queue
        where queue_name = x_queue_name
        for update;
    if not found then
    ...
    if x_force then
        perform pg_current.unregister_consumer(queue_name, consumer_name)
           from pg_current.get_consumer_info(x_queue_name);
    else
        -- check if no consumers
        select count(*) into num from pg_current.subscription
    ...
    for i in 0 .. (q.queue_ntables - 1) loop
        ...
        execute 'DROP TABLE ' || pg_current.quote_fqname(tblname);
    end loop;
    execute 'DROP TABLE ' || pg_current.quote_fqname(q.queue_data_pfx);

    -- delete ticks
    delete from pg_current.tick where tick_queue = q.queue_id;

    -- drop seqs
    -- FIXME: any checks needed here?
    execute 'DROP SEQUENCE ' || pg_current.quote_fqname(q.queue_tick_seq);
    execute 'DROP SEQUENCE ' || pg_current.quote_fqname(q.queue_event_seq);

    -- delete event
    delete from pg_current.queue
        where queue_name = x_queue_name;
...
create or replace function pg_current.drop_queue(x_queue_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.drop_queue(1)
--
--     Drop queue and all associated tables.
...
-- ----------------------------------------------------------------------
begin
    return pg_current.drop_queue(x_queue_name, false);
end;
$$ language plpgsql strict;


create or replace function pg_current.set_queue_config(
    x_queue_name    text,
    x_param_name    text,
    x_param_value   text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.set_queue_config(3)
--
--
...
begin
    ...
    perform 1 from pg_current.queue where queue_name = x_queue_name;
    if not found then
        raise exception 'No such event queue';
    end if;
    ...
    execute 'update pg_current.queue set ' 
...
create or replace function pg_current.insert_event(queue_name text, ev_type text, ev_data text)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.insert_event(3)
--
...
--      Event ID
-- Calls:
--      pg_current.insert_event(7)
-- ----------------------------------------------------------------------
begin
    return pg_current.insert_event(queue_name, ev_type, ev_data, null, null, null, null);
end;
$$ language plpgsql;



create or replace function pg_current.insert_event(
    queue_name text, ev_type text, ev_data text,
    ev_extra1 text, ev_extra2 text, ev_extra3 text, ev_extra4 text)
returns bigint as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.insert_event(7)
--
...
--      Event ID
-- Calls:
--      pg_current.insert_event_raw(11)
-- Tables directly manipulated:
--      insert - pg_current.insert_event_raw(11), a C function, inserts into current event_N_M table
-- ----------------------------------------------------------------------
begin
    return pg_current.insert_event_raw(queue_name, null, now(), null, null,
            ev_type, ev_data, ev_extra1, ev_extra2, ev_extra3, ev_extra4);
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

create or replace function pg_current.current_event_table(x_queue_name text)
returns text as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.current_event_table(1)
--
...
begin
    select queue_data_pfx || '_' || queue_cur_table::text,
        ...
        into res, disabled
        from pg_current.queue where queue_name = x_queue_name;
    if not found then
...
create or replace function pg_current.register_consumer(
    x_queue_name text,
    x_consumer_id text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.register_consumer(2)
--
...
-- Calls:
--      pg_current.register_consumer_at(3)
-- Tables directly manipulated:
--      None
-- ----------------------------------------------------------------------
begin
    return pg_current.register_consumer_at(x_queue_name, x_consumer_id, NULL);
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;


create or replace function pg_current.register_consumer_at(
    x_queue_name text,
    x_consumer_name text,
    x_tick_pos bigint)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.register_consumer_at(3)
--
...
--      None
-- Tables directly manipulated:
--      update/insert - pg_current.subscription
-- ----------------------------------------------------------------------
declare
    tmp         text;
    last_tick   bigint;
    ...
    sub record;
begin
    select queue_id into x_queue_id from pg_current.queue
        where queue_name = x_queue_name;
    if not found then
    ...
    select co_id into x_consumer_id from pg_current.consumer
        where co_name = x_consumer_name
        for update;
    if not found then
        insert into pg_current.consumer (co_name) values (x_consumer_name);
        x_consumer_id := currval('pg_current.consumer_co_id_seq');
    end if;

    -- if particular tick was requested, check if it exists
    if x_tick_pos is not null then
        perform 1 from pg_current.tick
    ...
    if found then
        if x_tick_pos is not null then
            ...
            update pg_current.subscription
    ...
    if x_tick_pos is null then
        ...
        select tick_id into last_tick from pg_current.tick
            where tick_queue = x_queue_id
            order by tick_queue desc, tick_id desc
            limit 1;
        if not found then
    ...
    -- register
    insert into pg_current.subscription (sub_queue, sub_consumer, sub_last_tick)
...
create or replace function pg_current.unregister_consumer(
    x_queue_name text,
    x_consumer_name text)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.unregister_consumer(2)
--
...
--      None
-- Tables directly manipulated:
--      delete - pg_current.retry_queue
--      delete - pg_current.subscription
-- ----------------------------------------------------------------------
declare
    x_sub_id integer;
    _sub_id_cnt integer;
...
begin
    select s.sub_id, c.co_id,
      ...
      from pg_current.subscription s, pg_current.consumer c, pg_current.queue q
    ...
    if _sub_id_cnt > 1 and _is_subconsumer then
        delete from pg_current.subscription
              where sub_id = x_sub_id
        ...
        perform 1 from pg_current.subscription
            where sub_consumer = _consumer_id;
        if not found then
            delete from pg_current.consumer
...
create or replace function pg_current.next_batch_info(
...
as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.next_batch_info(2)
--
...
-- Calls:
--      pg_current.next_batch_custom(5)
-- Tables directly manipulated:
--      None
-- ----------------------------------------------------------------------
begin
    select f.batch_id, f.cur_tick_id, f.prev_tick_id,
        ...
        from pg_current.next_batch_custom(i_queue_name, i_consumer_name, NULL, NULL, NULL) f;
    return;
end;
$$ language plpgsql;

create or replace function pg_current.next_batch(
    in i_queue_name text,
    in i_consumer_name text)
returns int8 as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.next_batch(2)
--
...
begin
    select batch_id into res
        from pg_current.next_batch_info(i_queue_name, i_consumer_name);
    return res;
end;
$$ language plpgsql;

create or replace function pg_current.next_batch_custom(
    in i_queue_name text,
    in i_consumer_name text,
...
as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.next_batch_custom(5)
--
...
begin
    select s.sub_queue, s.sub_consumer, s.sub_id, s.sub_batch,
        ...
        from pg_current.consumer c,
             pg_current.queue q,
             pg_current.subscription s
             left join pg_current.tick t1
                on (t1.tick_queue = s.sub_queue
                    and t1.tick_id = s.sub_last_tick)
             left join pg_current.tick t2
    ...
    if i_min_interval is null and i_min_count is null then
        ...
        select next_tick_id, next_tick_time, next_tick_seq
          into cur_tick_id, cur_tick_time, cur_tick_event_seq
          from pg_current.find_tick_helper(queue_id, prev_tick_id,
    ...
    -- get next batch
    batch_id := nextval('pg_current.batch_id_seq');
    update pg_current.subscription
...
create or replace function pg_current.get_batch_events(
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_batch_events(1)
--
...
declare
    sql text;
begin
    sql := pg_current.batch_event_sql(x_batch_id);
...
create or replace function pg_current.get_batch_cursor(
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_batch_cursor(4)
--
...
--      List of events.
-- Calls:
--      pg_current.batch_event_sql(i_batch_id) - internal function which generates SQL optimised specially for getting events in this batch
-- ----------------------------------------------------------------------
declare
    _cname  text;
    _sql    text;
begin
    ...
    _sql := pg_current.batch_event_sql(i_batch_id);
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_batch_cursor(3)
--
--      Get events in batch using a cursor.
--
-- Parameters:
...
--      List of events.
-- Calls:
--      pg_current.get_batch_cursor(4)
-- ----------------------------------------------------------------------
begin
    for ev_id, ev_time, ev_txid, ev_retry, ev_type, ev_data,
        ev_extra1, ev_extra2, ev_extra3, ev_extra4
    in
        select * from pg_current.get_batch_cursor(i_batch_id,
...
create or replace function pg_current.event_retry(
    x_batch_id bigint,
    x_event_id bigint,
    x_retry_time timestamptz)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.event_retry(3a)
--
...
begin
    ...
    select x_retry_time, sub_queue,
           ...
           ev_type, ev_data, ev_extra1, ev_extra2, ev_extra3, ev_extra4
      from pg_current.get_batch_events(x_batch_id),
           pg_current.subscription
...
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.event_retry(3b)
--
...
begin
    ...
    return pg_current.event_retry(x_batch_id, x_event_id, new_retry);
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

create or replace function pg_current.batch_retry(
    i_batch_id bigint,
    i_retry_seconds integer)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.batch_retry(2)
--
...
begin
    ...
    select * into _s from pg_current.subscription where sub_batch = i_batch_id;
    if not found then
    ...
    select distinct _retry, _s.sub_queue,
      ...
      from pg_current.get_batch_events(i_batch_id) b
           left join pg_current.retry_queue rq
                  on (rq.ev_id = b.ev_id
...
create or replace function pg_current.finish_batch(
    x_batch_id bigint)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.finish_batch(1)
--
...
drop function if exists pg_current.get_queue_info();
drop function if exists pg_current.get_queue_info(text);

create or replace function pg_current.get_queue_info(
    out queue_name                  text,
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_queue_info(0)
--
--      Get info about all queues.
--
-- Returns:
--      List of pg_current.ret_queue_info records.
...
begin
    ...
    in select
        ...
        from pg_current.get_queue_info(null) f
    loop
        return next;
    end loop;
    return;
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_queue_info(1)
--
--      Get info about particular queue.
--
-- Returns:
--      One pg_current.ret_queue_info record.
--      contente same as forpg_current.get_queue_info() 
-- ----------------------------------------------------------------------
declare
...
begin
    ...
    loop
        ...
        ev_new = pg_current.seq_getval(_queue_event_seq) - _top_tick_event_seq;
...
create or replace function pg_current.get_consumer_info(
    out queue_name      text,
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_consumer_info(0)
--
--      Returns info about all consumers on all queues.
--
-- Returns:
--      See pg_current.get_consumer_info(2)
-- ----------------------------------------------------------------------
begin
    ...
    in
        select f.queue_name, f.consumer_name, f.lag, f.last_seen,
            ...
            from pg_current.get_consumer_info(null, null) f
    loop
        return next;
    end loop;
    return;
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_consumer_info(1)
--
...
begin
    ...
    in
        select f.queue_name, f.consumer_name, f.lag, f.last_seen,
               f.last_tick, f.current_batch, f.next_tick, f.pending_events
            from pg_current.get_consumer_info(i_queue_name, null) f
    loop
        return next;
    end loop;
    return;
...
returns setof record as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_consumer_info(2)
--
...
begin
    ...
    in
        select q.queue_name, c.co_name,
          ...
          from pg_current.queue q,
               pg_current.consumer c,
               pg_current.subscription s
               left join pg_current.tick t
...
create or replace function pg_current.get_batch_info(
    in x_batch_id       bigint,
    out queue_name      text,
    out consumer_name   text,
...
as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.get_batch_info(1)
--
...
begin
    select q.queue_name, c.co_name,
        ...
        from pg_current.subscription s, pg_current.tick cur, pg_current.tick prev,
             pg_current.queue q, pg_current.consumer c
        where s.sub_batch = x_batch_id
...
-- pg_current transformations: schema rename, txid→pg_* renames, search_path
-- ======================================================================


-- ----------------------------------------------------------------------
-- Function: pg_current.insert_event_raw(11)
--
...
--      Event ID.
-- ----------------------------------------------------------------------
create or replace function pg_current.insert_event_raw(
...
begin
    _qname := queue_name;
    select q.queue_id,
        pg_current.quote_fqname(q.queue_data_pfx || '_' || q.queue_cur_table::text) as cur_table_name,
        nextval(q.queue_event_seq) as next_ev_id,
        q.queue_disable_insert
    from pg_current.queue q where q.queue_name = _qname into qstate;
...
-- pg_current transformations: schema rename, search_path hardening
-- ======================================================================

create or replace function pg_current.jsontriga() returns trigger as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.jsontriga()
--
...
-- >   FOR EACH ROW EXECUTE PROCEDURE pg_current.jsontriga('qname');
--
-- Redirect trigger example:
-- >   CREATE TRIGGER triga_nimi BEFORE INSERT OR UPDATE ON customer
-- >   FOR EACH ROW EXECUTE PROCEDURE pg_current.jsontriga('qname', 'SKIP');
-- ----------------------------------------------------------------------
declare
    qname text;
    ev_type text;
...
begin
    ...
    if current_setting('session_replication_role') = 'local' then
        ...
        perform pg_current.insert_event(qname, ev_type, '{}', ev_extra1, ev_extra2, ev_extra3, ev_extra4);
        return null;
    end if;
    ...
    -- insert final values
    if do_insert then
        perform pg_current.insert_event(qname, ev_type, ev_data, ev_extra1, ev_extra2, ev_extra3, ev_extra4);
    end if;
...
create or replace function pg_current.logutriga() returns trigger as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.logutriga()
--
...
-- Regular listen trigger example:
-- >   CREATE TRIGGER triga_nimi AFTER INSERT OR UPDATE ON customer
-- >   FOR EACH ROW EXECUTE PROCEDURE pg_current.logutriga('qname');
--
-- Redirect trigger example:
-- >   CREATE TRIGGER triga_nimi BEFORE INSERT OR UPDATE ON customer
-- >   FOR EACH ROW EXECUTE PROCEDURE pg_current.logutriga('qname', 'SKIP');
-- ----------------------------------------------------------------------
declare
    qname text;
    ev_type text;
...
begin
    ...
    begin
        ...
        loop
            ...
            data_sql_buf := array_append(data_sql_buf,
                    'select pg_current._urlencode(' || quote_literal(attr.attname)
                    || ') || coalesce(''='' || pg_current._urlencode(' || valexp
...
create or replace function pg_current._urlencode(val text)
returns text as $$
...
create or replace function pg_current.sqltriga() returns trigger as $$
-- ----------------------------------------------------------------------
-- Function: pg_current.sqltriga()
--
...
-- pg_current transformations: pgq_reader/writer/admin → pg_current_reader/writer/admin
-- ======================================================================



grant usage on schema pg_current to public;

-- old default grants
grant select on table pg_current.consumer to public;
grant select on table pg_current.queue to public;
grant select on table pg_current.tick to public;
grant select on table pg_current.queue to public;
grant select on table pg_current.subscription to public;
grant select on table pg_current.event_template to public;
grant select on table pg_current.retry_queue to public;

-- ======================================================================
-- Section 6: pg_current additions (NEW — not derived from PgQ)
-- ======================================================================

-- pg_current-additions/config.sql
...
-- pg_current-additions/queue_max_retries.sql
-- Add queue_max_retries column to pg_current.queue
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
--
-- The queue table is defined in PgQ's tables.sql.  After the transformed
-- PgQ schema is installed, we add this pg_current-specific column.
...
-- pg_current-additions/lifecycle.sql
...
-- pg_current-additions/roles.sql
...
-- pg_current-additions/dlq.sql
-- pg_current dead letter queue (DLQ) -- table + helper functions
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
--
-- PgQ has a retry queue but no dead letter queue. pg_current adds one.
-- See SPECx.md section 4.5.

-- pg_current.dead_letter table
--
-- FK behavior: both dl_queue_id and dl_consumer_id use `on delete cascade`.
-- Rationale:
--   - `pg_current.drop_queue()` deletes the pg_current.queue row unconditionally. DLQ
...
--     `pg_current.dlq_purge` or copy the rows out before dropping the queue).
--   - `pg_current.unregister_consumer()` deletes the pg_current.consumer row only when
...
--     audit trail matters.
create table if not exists pg_current.dead_letter (
    dl_id           bigserial primary key,
    dl_queue_id     int4 not null references pg_current.queue(queue_id)    on delete cascade,
    dl_consumer_id  int4 not null references pg_current.consumer(co_id)    on delete cascade,
...
create index if not exists dl_queue_time_idx
    on pg_current.dead_letter (dl_queue_id, dl_time);

-- pg_current.event_dead() -- move event to DLQ (called by nack() when max retries exceeded)
create or replace function pg_current.event_dead(
    i_batch_id bigint,
    i_event_id bigint,
    i_reason text,
...
-- pg_current.dlq_inspect() -- inspect DLQ entries for a queue
create or replace function pg_current.dlq_inspect(
    i_queue_name text, i_limit_count int default 100)
returns setof pg_current.dead_letter as $$
begin
    return query
    select dl.*
    from pg_current.dead_letter dl
    join pg_current.queue q on q.queue_id = dl.dl_queue_id
    where q.queue_name = i_queue_name
...
-- pg_current.dlq_replay() -- replay a single dead letter event back into the queue
create or replace function pg_current.dlq_replay(i_dead_letter_id bigint)
returns bigint as $$
declare
    v_dl record;
    v_queue_name text;
...
begin
    ...
    v_new_eid := pg_current.insert_event(v_dl.queue_name, v_dl.ev_type, v_dl.ev_data,
        v_dl.ev_extra1, v_dl.ev_extra2, v_dl.ev_extra3, v_dl.ev_extra4);

    -- Remove from DLQ
    delete from pg_current.dead_letter where dl_id = i_dead_letter_id;

    return v_new_eid;
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

-- pg_current.dlq_replay_all() -- replay all DLQ events for a queue
create or replace function pg_current.dlq_replay_all(i_queue_name text)
returns integer as $$
declare
    v_dl record;
    v_cnt integer := 0;
begin
    for v_dl in
        ...
        where q.queue_name = i_queue_name
    loop
        perform pg_current.insert_event(v_dl.queue_name, v_dl.ev_type, v_dl.ev_data,
            v_dl.ev_extra1, v_dl.ev_extra2, v_dl.ev_extra3, v_dl.ev_extra4);
        delete from pg_current.dead_letter where dl_id = v_dl.dl_id;
        v_cnt := v_cnt + 1;
    end loop;
...
-- pg_current.dlq_purge() -- purge old DLQ entries
create or replace function pg_current.dlq_purge(
    i_queue_name text, i_older_than interval default '30 days')
returns integer as $$
declare
    v_cnt integer;
begin
    delete from pg_current.dead_letter
    where dl_queue_id = (select queue_id from pg_current.queue where queue_name = i_queue_name)
...
-- pg_current_admin.
--
-- dlq_inspect is read-only — available to pg_current_reader and above.
-- dlq_replay / dlq_replay_all re-insert events — writer-level.
-- dlq_purge / event_dead: admin-level operations (purge = data loss,
-- event_dead = internal DLQ hook called from nack()). Granted to pg_current_admin
...
grant select on pg_current.dead_letter                           to pg_current_reader;
grant all    on pg_current.dead_letter                           to pg_current_admin;
grant all    on sequence pg_current.dead_letter_dl_id_seq        to pg_current_admin;

grant execute on function pg_current.dlq_inspect(text, int)      to pg_current_reader;
grant execute on function pg_current.dlq_replay(bigint)          to pg_current_writer;
grant execute on function pg_current.dlq_replay_all(text)        to pg_current_writer;
grant execute on function pg_current.event_dead(
    bigint, bigint, text, timestamptz, xid8, int4,
    text, text, text, text, text, text)                     to pg_current_admin;
grant execute on function pg_current.dlq_purge(text, interval)   to pg_current_admin;

-- ======================================================================
-- Section 7: pg_current-api (NEW — not derived from PgQ)
-- ======================================================================

-- pg_current-api/maint.sql
-- pg_current maint() -- default maintenance runner for v0.1
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
--
...
-- transaction from step1 (PgQ design requirement). pg_current.start() schedules
-- step2 as its own pg_cron job.
create or replace function pg_current.maint()
returns integer as $$
declare
    f record;
    sql text;
    ...
    total integer := 0;
begin
    for f in select func_name, func_arg from pg_current.maint_operations()
    loop
        -- Skip step2: it needs a separate transaction (scheduled by pg_current.start)
        if f.func_name = 'pg_current.maint_rotate_tables_step2' then
            continue;
...
-- pg_current-api/receive.sql
-- pg_current.receive(), pg_current.ack(), pg_current.nack() -- modern consume API
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
-- Includes code derived from PgQ (ISC license, Marko Kreen / Skype Technologies OU).
--
...
-- pg_current.message type (idempotent creation)
do $$ begin
...
-- pg_current.receive() -- wraps next_batch + get_batch_events
create or replace function pg_current.receive(
...
-- pg_current.ack() -- finishes the batch, advances consumer position
create or replace function pg_current.ack(i_batch_id bigint)
returns integer as $$
begin
...
-- pg_current.nack() -- retry or route to DLQ based on retry_count vs max_retries
create or replace function pg_current.nack(
    i_batch_id bigint,
...
-- Grants
-- ---------------------------------------------------------------------------
-- Colocated here (not in pg_current-additions/roles.sql) because roles.sql is
-- assembled before pg_current-api/, so these functions do not yet exist when
-- roles.sql runs. Same convention as sql/pg_current-api/send.sql.

grant execute on function pg_current.receive(text, text, int)                      to pg_current_writer;
grant execute on function pg_current.ack(bigint)                                   to pg_current_writer;
grant execute on function pg_current.nack(bigint, pg_current.message, interval, text)   to pg_current_writer;

-- pg_current-api/send.sql
-- pg_current-api/send.sql -- Modern send/subscribe API layer
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
-- Includes code derived from PgQ (ISC license, Marko Kreen / Skype Technologies OU).
--
-- Implements default v0.1 API surface:
--   pg_current.message type
--   pg_current.send(queue, payload)                -- text + jsonb overloads
--   pg_current.send(queue, type, payload)          -- text + jsonb overloads
--   pg_current.send_batch(queue, type, payloads[]) -- text[] + jsonb[] overloads
--   pg_current.subscribe(queue, consumer)
--   pg_current.unsubscribe(queue, consumer)
--
...
--
--   select pg_current.send('orders', '{"k":1}');           -- picks send(text, text)
--   select pg_current.send('orders', '{"k":1}'::jsonb);    -- picks send(text, jsonb)
--
...
-- pg_current.send(queue, payload jsonb) -- send with default type, JSON payload
create or replace function pg_current.send(i_queue text, i_payload jsonb)
returns bigint as $$
begin
...
-- pg_current.send(queue, payload text) -- fast path, opaque textual payload
...
-- encode first.
create or replace function pg_current.send(i_queue text, i_payload text)
returns bigint as $$
begin
...
-- pg_current.send(queue, type, payload jsonb) -- send with explicit type, JSON payload
create or replace function pg_current.send(i_queue text, i_type text, i_payload jsonb)
returns bigint as $$
begin
...
-- pg_current.send(queue, type, payload text) -- fast path with explicit type
create or replace function pg_current.send(i_queue text, i_type text, i_payload text)
returns bigint as $$
begin
...
-- pg_current.send_batch(queue, type, payloads jsonb[]) -- batch send, JSON payloads
create or replace function pg_current.send_batch(
...
-- pg_current.send_batch(queue, type, payloads text[]) -- fast-path batch send
create or replace function pg_current.send_batch(
...
-- pg_current.subscribe(queue, consumer) -- wrapper for register_consumer
create or replace function pg_current.subscribe(i_queue text, i_consumer text)
returns integer as $$
begin
...
-- pg_current.unsubscribe(queue, consumer) -- wrapper for unregister_consumer
create or replace function pg_current.unsubscribe(i_queue text, i_consumer text)
returns integer as $$
begin
    return pg_current.unregister_consumer(i_queue, i_consumer);
end;
$$ language plpgsql security definer set search_path = pg_current, pg_catalog;

-- Explicit writer grants for the send* + subscribe/unsubscribe family.
-- Colocated here (not in pg_current-additions/roles.sql) because roles.sql
-- runs before the pg_current-api section during build, so API-layer grants
-- must live alongside the function definitions they apply to.
grant execute on function pg_current.send(text, jsonb)               to pg_current_writer;
grant execute on function pg_current.send(text, text)                to pg_current_writer;
grant execute on function pg_current.send(text, text, jsonb)         to pg_current_writer;
grant execute on function pg_current.send(text, text, text)          to pg_current_writer;
grant execute on function pg_current.send_batch(text, text, jsonb[]) to pg_current_writer;
grant execute on function pg_current.send_batch(text, text, text[])  to pg_current_writer;
grant execute on function pg_current.subscribe(text, text)           to pg_current_writer;
grant execute on function pg_current.unsubscribe(text, text)         to pg_current_writer;
