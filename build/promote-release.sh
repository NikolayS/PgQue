...
# Atomically promote generated PgQue SQL or restore the development version.
...
main() {
  ...
  if [[ "${mode}" == promote ]]; then
    if [[ "${current_version}" != "${version}-devel" \
      ...
      echo "FAIL: ${lifecycle} reports ${current_version};" \
        "expected ${version}-devel" >&2
      exit 1
    fi
  else
  ...
  if [[ "${mode}" == restore-devel ]]; then
    bash "${verifier}" "${stable_dir}"
    echo "Restored devel/sql to ${desired_version};" \
      "stable sql/ remains ${version}."
    exit 0
  fi
