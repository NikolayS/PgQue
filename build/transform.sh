...
# -- pg_tle packaging ---------------------------------------------------------
# sql/pgque-tle.sql is a small hand-maintained psql wrapper that reads
# sql/pgque.sql at runtime via `\set pgque_body \`cat sql/pgque.sql\``, so the
# 5,000-line install body is not duplicated in the repo. The validation below
# is a smoke test on the wrapper itself.

echo ""
echo "=== Validating sql/pgque-tle.sql wrapper ==="

PGTLE_FILE="${SQL_DIR}/pgque-tle.sql"
pgtle_errors=0

if [[ ! -f "${PGTLE_FILE}" ]]; then
  echo "FAIL: ${PGTLE_FILE} is missing"
  pgtle_errors=$((pgtle_errors + 1))
fi

if ! grep -q 'pgtle.install_extension(' "${PGTLE_FILE}"; then
  echo "FAIL: ${PGTLE_FILE} missing pgtle.install_extension() call"
  pgtle_errors=$((pgtle_errors + 1))
fi

if ! grep -q 'cat sql/pgque.sql' "${PGTLE_FILE}"; then
  echo "FAIL: ${PGTLE_FILE} no longer reads sql/pgque.sql via cat"
  pgtle_errors=$((pgtle_errors + 1))
fi

if [[ ${pgtle_errors} -eq 0 ]]; then
  echo "=== pg_tle WRAPPER OK ==="
else
  echo "=== pg_tle WRAPPER: ${pgtle_errors} CHECK(S) FAILED ==="
  exit 1
fi
