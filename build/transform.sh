...
# transform.sh -- Mechanical transformation of PgQ sources into pg_current
#
...
# pg_current includes code derived from PgQ (ISC license, Marko Kreen).
...
apply_schema_rename() {
  # Rename pgq schema references to pg_current.
  ...
  # 1. Role names: pgq_reader -> pg_current_reader, etc.
  content=$(echo "$content" | sed \
    -e "s/pgq_reader/pg_current_reader/g" \
    -e "s/pgq_writer/pg_current_writer/g" \
    -e "s/pgq_admin/pg_current_admin/g")

  # 2. Schema-qualified references: pgq.something -> pg_current.something
  #    Matches pgq. followed by a letter or underscore (not a digit boundary issue)
  content=$(echo "$content" | sed -E "s/pgq\\.([a-zA-Z_])/pg_current.\\1/g")

  # 3. String literals referencing the schema name:
  #    'pgq' -> 'pg_current' (used in information_schema queries, extname, etc.)
  content=$(echo "$content" | sed "s/'pgq'/'pg_current'/g")

  # 4. Standalone pgq as schema name in CREATE/DROP SCHEMA, GRANT ON SCHEMA, etc.
  #    "schema pgq" -> "schema pg_current"
  content=$(echo "$content" | sed -E "s/schema pgq([^a-zA-Z0-9_])/schema pg_current\\1/g")
  content=$(echo "$content" | sed -E "s/schema pgq$/schema pg_current/g")
...
apply_search_path_to_security_definer() {
  # Add SET search_path = pg_current, pg_catalog to SECURITY DEFINER functions
  ...
  content=$(echo "$content" | sed -E \
    's/^(\$\$ language plpgsql) security definer;(.*)$/\1 security definer set search_path = pg_current, pg_catalog;\2/')
...
echo "=== PgQ -> pg_current transformation pipeline ==="
echo "Source: ${PGQ_DIR}"
...
for src_file in "${SOURCE_FILES[@]}"; do
  ...
  out_file=$(echo "${src_file}" | sed 's/pgq\./pg_current./g')
...
remaining_hooks=$(grep -n 'pgq_node\|londiste' "${OUTPUT_DIR}/functions/pg_current.maint_operations.sql" \
  | grep -v '^[0-9]*:\s*--' \
  || true)
...
TICKER_FILE="${OUTPUT_DIR}/functions/pg_current.ticker.sql"

# Patch the ticker: inject pg_notify before return statements (awk for portability)
awk '
/^    return i_tick_id;$/ {
  print ""
  print "    -- pg_current: notify listeners after tick"
  print "    perform pg_notify('"'"'pg_current_'"'"' || i_queue_name, i_tick_id::text);"
}
/^    return currval\(q\.queue_tick_seq\);$/ {
  print ""
  print "    -- pg_current: notify listeners after tick"
  print "    perform pg_notify('"'"'pg_current_'"'"' || i_queue_name, currval(q.queue_tick_seq)::text);"
}
{ print }
...
sedi 's/Function: pg_current.logutriga()/Function: pg_current.sqltriga()/' "${OUTPUT_DIR}/lowlevel_pl/sqltriga.sql"

echo "PASS: sqltriga comment header fixed"

# Remove debug comments from ticker
sedi 's/ -- unsure about access//' "${OUTPUT_DIR}/functions/pg_current.ticker.sql"
...
BATCH_SQL_FILE="${OUTPUT_DIR}/functions/pg_current.batch_event_sql.sql"
...
# -- Assembly: build sql/pg_current.sql ------------------------------------

echo ""
echo "=== Assembling sql/pg_current.sql ==="

SQL_DIR="${REPO_ROOT}/sql"
INSTALL_FILE="${SQL_DIR}/pg_current.sql"
ADDITIONS_DIR="${SQL_DIR}/pg_current-additions"
...
# Schema creation
echo "create schema if not exists pg_current;" >> "${INSTALL_FILE}"
echo "" >> "${INSTALL_FILE}"
...
echo "--" >> "${INSTALL_FILE}"
echo "-- pg_current transformations applied:" >> "${INSTALL_FILE}"
echo "--   1. Schema rename: pgq → pg_current (all identifiers, grants, references)" >> "${INSTALL_FILE}"
...
# Remove the original "create schema pg_current;" line (we already have IF NOT EXISTS above)
tables_content=$(echo "$tables_content" | sed '/^create schema pg_current;$/d')
...
echo "--" >> "${INSTALL_FILE}"
echo "-- pg_current transformations applied:" >> "${INSTALL_FILE}"
echo "--   1. Schema rename: pgq → pg_current" >> "${INSTALL_FILE}"
...
echo "--   5. SECURITY DEFINER functions get SET search_path = pg_current, pg_catalog" >> "${INSTALL_FILE}"
...
FUNCTION_FILES=(
  pg_current.upgrade_schema.sql
  pg_current.batch_event_sql.sql
  pg_current.batch_event_tables.sql
  pg_current.event_retry_raw.sql
  pg_current.find_tick_helper.sql
  pg_current.ticker.sql
  pg_current.maint_retry_events.sql
  pg_current.maint_rotate_tables.sql
  pg_current.maint_tables_to_vacuum.sql
  pg_current.maint_operations.sql
  pg_current.grant_perms.sql
  pg_current.tune_storage.sql
  pg_current.force_tick.sql
  pg_current.seq_funcs.sql
  pg_current.quote_fqname.sql
  pg_current.create_queue.sql
  pg_current.drop_queue.sql
  pg_current.set_queue_config.sql
  pg_current.insert_event.sql
  pg_current.current_event_table.sql
  pg_current.register_consumer.sql
  pg_current.unregister_consumer.sql
  pg_current.next_batch.sql
  pg_current.get_batch_events.sql
  pg_current.get_batch_cursor.sql
  pg_current.event_retry.sql
  pg_current.batch_retry.sql
  pg_current.finish_batch.sql
  pg_current.get_queue_info.sql
  pg_current.get_consumer_info.sql
  pg_current.get_batch_info.sql
)
...
echo "-- pg_current transformations: schema rename, txid→pg_* renames, search_path" >> "${INSTALL_FILE}"
echo "-- ======================================================================" >> "${INSTALL_FILE}"
echo "" >> "${INSTALL_FILE}"
...
echo "-- pg_current transformations: schema rename, search_path hardening" >> "${INSTALL_FILE}"
echo "-- ======================================================================" >> "${INSTALL_FILE}"
echo "" >> "${INSTALL_FILE}"
...
echo "-- pg_current transformations: pgq_reader/writer/admin → pg_current_reader/writer/admin" >> "${INSTALL_FILE}"
echo "-- ======================================================================" >> "${INSTALL_FILE}"
echo "" >> "${INSTALL_FILE}"
...
# Section 6: pg_current additions
echo "-- ======================================================================" >> "${INSTALL_FILE}"
echo "-- Section 6: pg_current additions (NEW — not derived from PgQ)" >> "${INSTALL_FILE}"
echo "-- ======================================================================" >> "${INSTALL_FILE}"
echo "" >> "${INSTALL_FILE}"

for addition_file in config.sql queue_max_retries.sql lifecycle.sql roles.sql dlq.sql; do
  echo "-- pg_current-additions/${addition_file}" >> "${INSTALL_FILE}"
  cat "${ADDITIONS_DIR}/${addition_file}" >> "${INSTALL_FILE}"
  echo "" >> "${INSTALL_FILE}"
done

# Section 7: pg_current-api (default v0.1 API surface)
API_DIR="${SQL_DIR}/pg_current-api"
if [[ -d "${API_DIR}" ]]; then
  echo "-- ======================================================================" >> "${INSTALL_FILE}"
  echo "-- Section 7: pg_current-api (NEW — not derived from PgQ)" >> "${INSTALL_FILE}"
  echo "-- ======================================================================" >> "${INSTALL_FILE}"
  echo "" >> "${INSTALL_FILE}"
  ...
  for api_name in "${DEFAULT_API_FILES[@]}"; do
    ...
    if [[ -f "${api_file}" ]]; then
      echo "-- pg_current-api/$(basename "${api_file}")" >> "${INSTALL_FILE}"
...
# Add "pg_current transformation:" comments to specific lines in the output.
...
sedi '/queue_switch_step1.*default pg_current_xact_id/s/$/ -- pg_current transformation: txid_current()→pg_current_xact_id()::text::bigint (PG14+)/' "${INSTALL_FILE}"

sedi '/ev_txid.*xid8.*default pg_current_xact_id/s/$/ -- pg_current transformation: bigint→xid8 (needed for pg_visible_in_snapshot)/' "${INSTALL_FILE}"

# LISTEN/NOTIFY injection (appears twice in ticker)
sedi '/perform pg_notify.*pg_current_/s/$/ -- pg_current transformation: LISTEN\/NOTIFY wakeup (not in original PgQ)/' "${INSTALL_FILE}"

# search_path pinning — annotate first occurrence only via awk
awk '/set search_path = pg_current, pg_catalog;/ && !sp_done {
  sp_done=1; sub(/;/, "; -- pg_current transformation: pin search_path (SECURITY DEFINER hardening)")
...
if head -1 "${INSTALL_FILE}" | grep -q 'pg_current.sql'; then
...
if grep -q 'create schema if not exists pg_current;' "${INSTALL_FILE}"; then
...
# Verify pg_current additions and API layer are in the script
if grep -q 'lifecycle.sql\|pg_current.version\|pg_current.start\|pg_current.stop' "${INSTALL_FILE}"; then
  echo "PASS: pg_current additions present in install script"
else
  echo "FAIL: pg_current additions not found in install script"
  asm_errors=$((asm_errors + 1))
fi

# Verify default pg_current-api functions are in the script
if grep -q 'pg_current.receive\|pg_current.ack\|pg_current.send\|pg_current.subscribe\|pg_current.maint' "${INSTALL_FILE}"; then
  echo "PASS: default pg_current-api surface present in install script"
else
  echo "FAIL: default pg_current-api surface not found in install script"
  asm_errors=$((asm_errors + 1))
fi

# Verify experimental APIs are NOT in the default install script.
# DLQ (dlq_inspect / event_dead / pg_current.dead_letter) was promoted to the default
...
# Verify pg_current-api section is present (if default api files exist)
if [[ -d "${API_DIR}" ]]; then
  if grep -q 'Section 7: pg_current-api' "${INSTALL_FILE}"; then
    echo "PASS: pg_current-api section present in install script"
  else
    echo "FAIL: pg_current-api section missing from install script"
    asm_errors=$((asm_errors + 1))
  fi
fi
