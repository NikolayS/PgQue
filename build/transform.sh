...
# anyone reading devel/sql/pgque.sql understands why.
...
# -- Assembly: build devel/sql/pgque.sql ------------------------------------

echo ""
echo "=== Assembling sql/pgque.sql ==="

# The build emits into devel/sql/ -- the in-development version. The stable,
# already-released install artifacts live in sql/ and are frozen (not touched
# by this script). At release time, the devel/sql/ artifacts are promoted to
# sql/. See devel/sql/README.md.
SQL_DIR="${REPO_ROOT}/devel/sql"
SQL_REL="${SQL_DIR#"${REPO_ROOT}/"}"
...
# devel/sql/pgque.sql header so the two can never drift apart.

cat > "${PGTLE_FILE}" << HEADER
-- pgque-tle.sql -- Install PgQue as a pg_tle (Trusted Language Extension).
-- Auto-generated by build/transform.sh from ${SQL_REL}/pgque.sql. Do not edit by hand.
-- Copyright 2026 Nikolay Samokhvalov. Apache-2.0 license.
-- Includes code derived from PgQ (ISC license, Marko Kreen / Skype Technologies OU).
--
...
-- passing the contents of ${SQL_REL}/pgque.sql as the extension body.
--
-- Prerequisites:
...
--
-- Usage:
--   psql -d mydb -f ${SQL_REL}/pgque-tle.sql
--   psql -d mydb -c 'create extension pgque;'
--
-- Uninstall:
--   psql -d mydb -f ${SQL_REL}/pgque-tle-uninstall.sql
