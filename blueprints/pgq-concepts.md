# PgQ: Glossary and Concepts

Reference vocabulary and semantics for PgQue contributors, and a source of
quotable phrasing for user-facing docs. Definitions are adapted from the
2009 PgCon talk "Skytools: PgQ -- Queues and applications" by Marko Kreen
and Martin Pihlak
...
Additional background in `pgq-history.md`.

## Core feature claims

Lightly adapted from the 2009 talk:
...
- **Flexible.** No limits on the number of producers or consumers. Custom
  event formats.
- **Reliable.** Events live in PostgreSQL, inheriting write-ahead logging
  and crash recovery.
...
These definitions are the authoritative vocabulary for PgQue docs. Reuse
them verbatim where possible rather than rewording.

### Event

An atomic piece of data created by a producer. Physically, an event is one
row in a queue table.

**Guarantee:** PgQ guarantees that each event is seen *at least once*. It
is up to the consumer to ensure the event is processed *no more than once*
when exactly-once semantics matter.

### Batch

A group of events served to a consumer together. PgQ is designed for
efficiency and high throughput, so events are processed in batches rather
than one at a time. Batch size is tunable: larger batches for WAN
consumers, smaller batches for low-latency local processing.

Small batches incur higher per-event overhead; excessively large batches
have their own disadvantages (memory pressure, long processing windows,
retry cost).

### Queue

A named stream of events, physically a set of tables in a PostgreSQL
database. The default is **3 rotating tables per queue**, which allows
discarded events to be purged by `TRUNCATE` rather than `DELETE` +
`VACUUM`.
...
Any application that places events into a queue. A producer can be written
in any language that can execute SQL against PostgreSQL.
...
Any application that reads events from a queue. Can be written in any
language that can talk to PostgreSQL.
...
A daemon-or-scheduled-job that periodically creates **ticks** on each
queue. A tick is a position marker in the event stream; a batch is the set
of events enqueued between two consecutive ticks.
...
produce a huge batch, consumers might not be able to cope with it. Keep
the ticker running!" -- Kreen & Pihlak, PgCon 2009.

In PgQue, the ticker runs via `pg_cron` by default, calling
`pgque.ticker()`.

### Tick

A single position in the event stream, created by the ticker. Ticks
delimit batches.

### Cooperative consumer (sub-consumer)

A Skytools 3 concept: multiple sub-consumers share the workload of a
single logical consumer so events are not processed twice. Useful when one
consumer cannot keep up with the volume. PgQue's equivalent lives in the
`pgque-api` layer.
...
replication failover. Not part of pgque-core; documented here for
...
"Field names hint at their intended usage" -- Kreen & Pihlak, PgCon 2009. The content
format of `ev_data` / `ev_type` is agreed between producer and consumer;
PgQ / PgQue does not interpret it.
...
2. **Cross-database processing.** Use batch / event tracking on the target
   ...
   equivalent in `pgque-api`). Idempotency is guaranteed by skipping
   batches already marked done on the target before committing on the
   source.
...
1. `next_batch(queue, consumer)` -> batch id, or `NULL` if nothing is
   ready.
2. `get_batch_events(batch_id)` -> the event set (may be empty if the tick
   interval had no events).
3. Process the events. Optionally call `event_retry(batch_id, ev_id, sec)`
   to postpone an individual event.
...
- **lag** -- age of the last finished batch. High lag means the consumer
  is falling behind.
- **last_seen** -- elapsed time since the consumer processed any batch.
  High last_seen means the consumer process is not running (or is stuck).
...
`queue_rotation_period`, ...). They are consulted by whatever process
calls `pgque.ticker()` -- in PgQue, that is `pg_cron`. They are *not* tied
to the historical `pgqd` / `pgqadm.py` daemons (which we do not ship);
those tools were just client-side front-ends that wrote the same DB
columns.

- `ticker_max_lag` -- max wall time between ticks.
- `ticker_idle_period` -- tick interval when no events are arriving.
- `ticker_max_count` -- force a tick when this many events have
  accumulated. Effectively a batch-size cap.
