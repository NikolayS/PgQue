...
> **UPDATE 2026-05-02:** For PR #159, the `send_batch(queue, type, payloads[])` row above is superseded by `send_batch(queue_name, type_name, payloads)`: one queue lookup plus one set-based insert via internal `insert_event_bulk()`; `text[]` remains the default overload and `jsonb[]` remains opt-in via `::jsonb[]`. The new argument names are intentional because PostgreSQL supports named-argument calls with `arg := value`.
