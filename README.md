<h1 align="center">pg_current — event log for Postgres</h1>

<p align="center"><strong>Zero-bloat event log, PgQ heritage. One SQL file to install, <code>pg_cron</code> to tick.</strong></p>

<p align="center">
  <a href="https://github.com/NikolayS/pg_current/actions/workflows/ci.yml"><img src="https://github.com/NikolayS/pg_current/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  ...
  <a href="https://github.com/NikolayS/pg_current"><img src="https://img.shields.io/badge/anti--extension-%5Ci_and_go-orange" alt="Anti-Extension"></a>
...
<p align="center"><img src="docs/images/death_spiral.gif" alt="Death spiral of a SKIP LOCKED queue under sustained load — the failure mode pg_current avoids by construction" width="720"></p>
...
pg_current brings back [PgQ](https://github.com/pgq/pgq) — one of the longest-running Postgres queue architectures in production — in a form that runs on any Postgres platform, managed providers included.
...
pg_current rebuilds that battle-tested engine in pure PL/pgSQL, so the zero-bloat queue pattern works anywhere you can run SQL — without adding another distributed system to your stack.
...
pg_current avoids that whole class of problems. It uses **snapshot-based batching** and **TRUNCATE-based table rotation** instead of per-row deletion. The hot path stays predictable:
...
pg_current gives you queue semantics **inside** Postgres, with Postgres durability and transactional behavior, without the bloat tax most in-database queues eventually hit.

## Latency trade-off

pg_current is built around **snapshot-based batching**, not row-by-row claiming. That's what gives it zero bloat in the hot path, stable behavior under sustained load, and clean ACID semantics inside Postgres.
...
If your top priority is single-digit-millisecond dispatch, pg_current is the wrong tool. If your priority is **stability under load without bloat**, that is where pg_current fits.

## Comparison

| Feature | pg_current | PgQ | PGMQ | River | Que | pg-boss |
|---|---|---|---|---|---|---|
...
- **[PgQ](https://github.com/pgq/pgq)** is the Skype-era queue engine (~2007) pg_current is derived from. Same snapshot/rotation architecture, but requires C extensions and an external daemon (`pgqd`) — unavailable on managed Postgres. pg_current removes both constraints.
- **No external daemon:** pg_current uses pg_cron (or your own scheduler) for ticking; PGMQ uses visibility timeouts. River, Que, and pg-boss require a Go / Ruby / Node.js worker binary.
...
- **Category:** River, Que, and pg-boss (and Oban, graphile-worker, solid_queue, good_job) are **job queue frameworks**. pg_current is an **event/message queue** optimized for high-throughput streaming with fan-out.

### What differentiates pg_current
...
Oban Pro shipped table partitioning to mitigate it; PGMQ ships aggressive autovacuum settings. pg_current's TRUNCATE rotation creates zero dead tuples by construction. No tuning. Immune to xmin horizon pinning.

**2. Native fan-out.** Each registered consumer maintains its own cursor on a shared event log and independently receives all events. That is different from competing-consumers (SKIP LOCKED) where each job goes to one worker. pg-boss has fan-out but it is copy-per-queue (one INSERT per subscriber per event). pg_current's model is a position on a shared log — no data duplication, atomic batch boundaries, late subscribers catch up. Closer to Kafka topics than to a job queue.

### Log, not task queue (but can serve task-queue workloads)

pg_current is a log. It can still serve task-queue workloads when the workload fits a log's shape.
...
For bad-fit workloads, use a task-queue library (River, graphile-worker, Oban, pgmq) or an external broker (RabbitMQ, ActiveMQ, SQS). For good-fit workloads, pg_current saves you from running a second system.

## Installation

**Requirements:** Postgres 14+, and something that calls `pg_current.ticker()` periodically (every 1 second by default). `pg_cron` is the recommended default — pre-installed or one-command available on all major managed Postgres providers (RDS, Aurora, Cloud SQL, AlloyDB, Supabase, Neon); on self-managed Postgres, follow the [pg_cron setup guide](https://github.com/citusdata/pg_cron#setting-up-pg_cron). Any external scheduler (system `cron`, systemd, a worker loop in your app) works as an alternative — see below.

Inside a psql session:

```sql
begin;
\i sql/pg_current.sql
commit;
```
...
```bash
PAGER=cat psql --no-psqlrc --single-transaction -d mydb -f sql/pg_current.sql
```

With `pg_cron` available in the same database as pg_current, `pg_current.start()` creates the default ticker and maintenance jobs:

```sql
select pg_current.start();
```

**pg_cron in a different database.** `pg_cron` runs jobs in one designated database (`cron.database_name`, typically `postgres`). If your pg_current schema lives in a different database, use the [cross-database pattern](https://github.com/citusdata/pg_cron#creating-a-cron-job-in-a-different-database) to call `pg_current.ticker()` and `pg_current.maint()` across databases. *Todo: a future release will detect this and emit the correct `cron.schedule_in_database` calls from `pg_current.start()` automatically.*
...
Without `pg_cron`, pg_current still installs. Drive ticking and maintenance from your application or an external scheduler:

```bash
PAGER=cat psql --no-psqlrc -c "select pg_current.ticker()"   # every 1 second
PAGER=cat psql --no-psqlrc -c "select pg_current.maint()"    # every 30 seconds
```

**Important:** pg_current does not deliver messages without a working ticker. Enqueueing still works, but consumers will see nothing new because no ticks are created. If you do not use `pg_cron`, run `pg_current.ticker()` and `pg_current.maint()` yourself.

Treat installation as one-way for now — upgrade and reinstall paths are still being tightened. To uninstall: `\i sql/pg_current_uninstall.sql`.
...
|---|---|---|
| `pg_current_reader` | Dashboards, metrics, debugging | `get_queue_info`, `get_consumer_info`, `get_batch_info`, `version`, plus `select` on all tables |
| `pg_current_writer` | Producers and consumers (most apps) | inherits `pg_current_reader` + the modern API (`send`, `send_batch`, `subscribe`, `unsubscribe`, `receive`, `ack`, `nack`) and the underlying PgQ primitives (`insert_event`, `next_batch`, `get_batch_events`, `finish_batch`, `event_retry`, `register_consumer`, `unregister_consumer`) |
| `pg_current_admin`  | Operators, migrations | inherits `pg_current_writer` + full schema/table/sequence access. `uninstall()` is revoked from both `pg_current_admin` and PUBLIC (superuser-only — it drops the schema). |

Typical app setup:

```sql
\i sql/pg_current.sql
select pg_current.start();                     -- optional pg_cron ticker + maint

create user app_orders with password '...';          -- replace with a real password
grant pg_current_writer to app_orders;

create user metrics with password '...';              -- replace with a real password
grant pg_current_reader to metrics;
```

DDL-class operations (`create_queue`, `drop_queue`, `start`, `stop`, `maint`, `ticker`, `force_tick`) are not granted to `pg_current_writer` and should be performed by an admin / migration role. They currently default to PUBLIC; revoking from PUBLIC and granting only to `pg_current_admin` is on the roadmap.

## Project status

pg_current is **early-stage** as a product and API layer. PgQ itself has run at Skype scale for over a decade. What's new here is the packaging, modernization, managed-Postgres compatibility, and the higher-level pg_current API around that core.
...
select pg_current.subscribe('orders', 'processor');
...
Send, tick, and receive should be separate transactions — that's PgQ's snapshot-based design working as intended. In normal operation, `pg_cron` or an external scheduler drives `pg_current.ticker()`; `force_tick()` is mainly for demos, tests, and manual operation.
...
pg_current is SQL-first, so any Postgres driver works. Example client libraries exist for **Python**, **Go**, and **TypeScript** — unpublished, still evolving, demonstrating integration patterns rather than stable SDKs. **Contributions welcome.**

### Python (`pg_current-py`) — psycopg 3

```python
from pg_current import PgqueClient, Consumer
...
### TypeScript (`pg_current-ts`) — node-postgres
...
pg_current includes code derived from [PgQ](https://github.com/pgq/pgq) (ISC license, Marko Kreen / Skype Technologies OU). See [NOTICE](NOTICE).
